"""
DSC 20 Project
Name(s): Joey Sandoval, Colin Tran
PID(s):  A18515990, A18488919
Sources: None
"""
...
class RGBImage:
        ...
        for row in pixels:
            ...
            for pixel in row:
                if not isinstance(pixel, list) or len(pixel) != 3:
                    raise TypeError()
                if any(not isinstance(val, int) or not (0 <= val <= 255) for val in pixel):
                    raise ValueError()
    ...
    def size(self):
        ...
        return tuple([self.num_rows, self.num_cols])
    ...
    def get_pixels(self):
        ...
        return [[[x for x in sublist] for sublist in sub2d] for sub2d in self.pixels]
    ...
    def get_pixel(self, row, col):
        ...
        if row < 0 or row >= self.num_rows or col < 0 or col >= self.num_cols:
            raise ValueError()
        red = self.pixels[row][col][0]
        blue = self.pixels[row][col][1]
        green = self.pixels[row][col][2]
    ...
    def set_pixel(self, row, col, new_color):
        ...
        if not isinstance(new_color, tuple) or len(new_color) != 3:
            raise TypeError()
        if any(not isinstance(val, int) for val in new_color):
            raise TypeError()
...
class ImageProcessingTemplate:
    ...
    def negate(self, image):
        ...
        """
        # YOUR CODE GOES HERE #
        new_pixels = [[[255 - val for val in pixel] for pixel in row] for row in image.get_pixels()]
    ...
    def grayscale(self, image):
        ...
        """
        # YOUR CODE GOES HERE #
        new_pixels = [[[sum(pixel) // 3]*3 for pixel in row] for row in image.get_pixels()]
        return RGBImage(new_pixels)
    ...
    def rotate_180(self, image):
        ...
        rotated = [row[::-1] for row in pixels[::-1]] 
    ...
    def get_average_brightness(self, image):
        ...
        pix_avg = [sum(pixel) // 3 for row in pixels for pixel in row]

        sum_pix_avg = sum(pix_avg)

        pix_count = len(pix_avg)

        avg_bright = sum_pix_avg // pix_count
    ...
    def adjust_brightness(self, image, intensity):
        ...
        new_pixels = [[[max(0, min(int(channel * intensity), 255)) for channel in pixel] for pixel in row] for row in pixels]
...
class StandardImageProcessing(ImageProcessingTemplate):
    ...
    def redeem_coupon(self, amount):
        ...
        if not isinstance(amount, int):
            raise TypeError("Coupon amount must be an integer")
        if amount <= 0:
            raise ValueError("Coupon amount must be positive")
...
class PremiumImageProcessing(ImageProcessingTemplate):
    ...
    def pixelate(self, image, block_dim):
        ...
        new_pixels = [[None] * width for i in range(height)]
        ...
        for i in range(0, height, block_dim):
            for j in range(0, width, block_dim):
                ...
                for x in range(i, i + block_dim):
                    for y in range(j, j + block_dim):
    ...
    def edge_highlight(self, image):
        ...
        for sub2d in image.get_pixels():
            averaged_row = []
            for sublst in sub2d:
                avg = sum(sublst) // len(sublst)
                averaged_row.append(avg)
            pixel_single_vals.append(averaged_row)
            ...
            for j in range(width):
                ...
                for x in range(-1, 2):  
                    for y in range(-1, 2):
                        ix = i + x
                        jy = j + y
                        if 0 <= ix < height and 0 <= jy < width:  
                            masked_val += pixel_single_vals[ix][jy] * kernel[x + 1][y + 1]




                masked_val = max(0, min(masked_val, 255))




                new_pixels[i][j] = [masked_val] * 3
...
class PremiumPlusMultimediaProcessing(PremiumImageProcessing):
    ...
    def reverse_song(self, audio):
        ...
        """
        # YOUR CODE GOES HERE #
        if type(audio) != AudioWave:
            raise TypeError()
        #start stop step
        reversed_audio = [audio.wave[i] for i in range(len(audio.wave)-1, -1, -1)]
        return AudioWave(reversed_audio)
    ...
    def slow_down(self, audio, factor):
        ...
        return AudioWave([i for i in audio.wave for j in range(factor)])
    ...
    def clip_song(self, audio, start, end):
        ...
        if not isinstance(audio, AudioWave):
            raise TypeError("audio must be an instance of AudioWave")
        if not isinstance(start, int) or not isinstance(end, int):
            raise TypeError("start and end must be integers")
        if not (0 <= start <= 100) or not (0 <= end <= 100):
            raise ValueError("start and end must be between 0 and 100")
        if end <= start:
            return AudioWave([])
        total_samples = len(audio.wave)
        start_idx = round((start * total_samples) // 100)
        end_idx = round((end * total_samples) // 100)
...
class ImageKNNClassifier:
    ...
    def __init__(self, k_neighbors):
        ...
        if k_neighbors <= 0:
            raise ValueError("k_neighbors must be a positive interger")
    ...
    def fit(self, data):
        ...
        if not isinstance(data, list):
            raise TypeError("data must be a list of (image, label) tuples")
        if any(not isinstance(pair, tuple) or len(pair) != 2 for pair in data):
            raise TypeError("Each item in data must be a tuple (image, label)")
        if len(data) < self.k_neighbors:
            raise ValueError("Number of training samples must be at least k_neighbors")
    ...
    def distance(self, image1, image2):
        ...
        flat1 = [channel for row in pixels1 for pixel in row for channel in pixel]
        flat2 = [channel for row in pixels2 for pixel in row for channel in pixel]

        squared_diff = sum((a - b) ** 2 for a, b in zip(flat1, flat2))
    ...
    def vote(self, candidates):
        ...
        """
        # YOUR CODE GOES HERE #
        count_dict = {label: candidates.count(label) for label in set(candidates)}
    ...
    def predict(self, image):
        ...
        distances = [(self.distance(image, train_image), label) for train_image, label in self.data]

        distances.sort(key=lambda x: x[0])
        k_nearest_labels = [label for i, label in distances[:self.k_neighbors]]
